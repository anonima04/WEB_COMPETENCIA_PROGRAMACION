...
export const createPerson = async (req, res) => {
  ...
  console.log(result);
  res.json({
    pk_id_persona: result.recordset[0].pk_id_persona,
...
export const updatePerson = async (req, res) => {
  ...
  const result = await pool
    ...
    .input("numero_documento", sql.NChar, req.body.numero_documento)
    .query(
      "UPDATE persona SET prim_nombre = @prim_nombre, seg_nombre= @seg_nombre, prim_apellido = @prim_apellido, seg_apellido= @seg_apellido, fk_id_documento= @fk_id_documento, numero_documento= @numero_documento WHERE pk_id_persona = @id"
    );
